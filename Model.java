...
public class Model {
  ...
  public Model(String name, Mesh mesh, Mat4 modelMatrix, Shader shader, Material material, Light[] light, Camera camera, Texture diffuse, Texture specular) {
    ...
    if(specular!=null){
    System.err.println("specular object"+specular.toString());
    }
  }
  ...
  public void render(GL3 gl, Mat4 modelMatrix) {
    ...
    shader.setInt(gl,"numLights", light.length);

    for (int i=0; i<light.length; i++) {
      shader.setVec3(gl, "lights["+i+"].position", light[i].getPosition());
      shader.setVec3(gl, "lights["+i+"].ambient", light[i].getMaterial().getAmbient());
      shader.setVec3(gl, "lights["+i+"].diffuse", light[i].getMaterial().getDiffuse());
      shader.setVec3(gl, "lights["+i+"].specular", light[i].getMaterial().getSpecular());
    }
  ...
  public void setMove(GL3 gl, double elapsed) {
    shader.use(gl);
    float offsetX = 0.0f;
    float offsetY = (float)(elapsed - Math.floor(elapsed))*0.17f;
